...
The RS Menu Surface component is a reusable surface that appears above the content of the
...
```css
@import "@material/menu-surface/rs-menu-surface";
```

### JavaScript Instantiation

```js
import {RSMenuSurface} from '@material/menu-surface';

const menuSurface = new RSMenuSurface(document.querySelector('.rs-menu-surface'));
```
...
```html
<div class="rs-menu-surface--anchor">
...
```html
<div class="rs-menu-surface rs-menu-surface--fixed">
...
</div>
```
...
--- | ---
`rs-menu-surface` | Mandatory.
`rs-menu-surface--animating-open` | Indicates the menu surface is currently animating open. This class is removed once the animation completes.
`rs-menu-surface--open` | Indicates the menu surface is currently open, or is currently animating open.
`rs-menu-surface--animating-closed` | Indicates the menu surface is currently animating closed. This class is removed once the animation completes.
`rs-menu-surface--anchor` | Used to indicate which element the menu should be anchored to.
`rs-menu-surface--fixed` | Used to indicate that the menu is using fixed positioning.

### Sass Mixins

Mixin | Description
--- | ---
`rs-menu-surface-ink-color($color)` | Sets the `color` property of the `rs-menu-surface`.
`rs-menu-surface-fill-color($color)` | Sets the `background-color` property of the `rs-menu-surface`.
`rs-menu-surface-shape-radius($radius, $rtl-reflexive)` | Sets the rounded shape to menu surface with given radius size. Set `$rtl-reflexive` to true to flip radius values in RTL context, defaults to false.
...
--- | ---
`RSMenuDimensions` | Width/height of an element. See [types.ts](./types.ts).
`RSMenuDistance` | Margin values representing the distance from anchor point that the menu surface should be shown. See [types.ts](./types.ts).
`RSMenuPoint` | X/Y coordinates. See [types.ts](./types.ts).

## `RSMenuSurface` Properties and Methods
...
`anchorElement` | `Element` | Gets or sets the element that the surface is anchored to, or `null` if the surface is not anchored. Defaults to the root element's parent `rs-menu-surface--anchor` element if present.

Method Signature | Description
--- | ---
`setAnchorCorner(Corner) => void` | Proxies to the foundation's `setAnchorCorner(Corner)` method.
`setAnchorMargin(Partial<RSMenuDistance>) => void` | Proxies to the foundation's `setAnchorMargin(Partial<RSMenuDistance>)` method.
`setFixedPosition(isFixed: boolean) => void` | Adds the `rs-menu-surface--fixed` class to the `rs-menu-surface` element. Proxies to the foundation's `setIsHoisted()` and `setFixedPosition()` methods.
...
`getDefaultFoundation() => RSMenuSurfaceFoundation` | Returns the foundation.

### Events

Event Name | Data | Description
--- | --- | ---
`RSMenuSurface:closed` | none | Event emitted after the menu surface is closed.
`RSMenuSurface:opened` | none | Event emitted after the menu surface is opened.
...
`isElementInContainer(el: Element) => boolean` | Returns true if the `el` Element is inside the `rs-menu-surface` container.
...
`getInnerDimensions() => RSMenuDimensions` | Returns an object with the items container width and height.
`getAnchorDimensions() => ClientRect \| null` | Returns an object with the dimensions and position of the anchor.
`getBodyDimensions() => RSMenuDimensions` | Returns an object with width and height of the body, in pixels.
`getWindowDimensions() => RSMenuDimensions` | Returns an object with width and height of the viewport, in pixels.
`getWindowScroll() => RSMenuPoint` | Returns an object with the amount the body has been scrolled on the `x` and `y` axis.
`setPosition(position: Partial<RSMenuDistance>) => void` | Sets the position of the menu surface element.
`setMaxHeight(value: string) => void` | Sets `max-height` style for the menu surface element.

### `RSMenuSurfaceFoundation`
...
`setAnchorMargin(margin: Partial<RSMenuDistance>) => void` | Sets the distance from the anchor point that the menu surface should be shown.
