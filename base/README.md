...
RS Base contains core foundation and component classes that serve as the base classes for all of RS Web's foundation classes and components (respectively).

Most of the time, you shouldn't need to depend on `rs-base` directly. It is useful however if you'd like to write custom components that follow RS Web's pattern and elegantly integrate with the RS Web ecosystem.
...
```javascript
import {RSComponent, RSFoundation} from '@material/base';
```
#### CommonJS

```javascript
const RSComponent = require('rs-base').RSComponent;
const RSFoundation = require('rs-base').RSFoundation;
```

#### AMD

```javascript
require(['path/to/rs-base'], function(mdcBase) {
  const RSComponent = mdcBase.RSComponent;
  const RSFoundation = mdcBase.RSFoundation;
});
```
...
```javascript
const RSComponent = mdc.base.RSComponent;
const RSFoundation = mdc.base.RSFoundation;
```

## Usage

rs-base exposes two classes: `RSComponent` (the default export) which all components extend from, and `RSFoundation`, which all foundation classes extend from. To learn more about foundation classes vs. components, check out our overview on [architecture and best practices](../../docs/code).

### RSFoundation

RSFoundation provides the basic mechanisms for implementing foundation classes. Subclasses are expected to:
...
export default class MyFoundation extends RSFoundation {
...
RSComponent provides the basic mechanisms for implementing component classes.

```javascript
import MyComponentFoundation from './foundation';

export class MyComponent extends RSComponent {
...
`RSComponent` provides the following "private" properties to subclasses:
...
`RSComponent` provides the following methods to subclasses:
...
| method | description |
| --- | --- |
| `attachTo(root) => <ComponentClass>` | Subclasses must implement this as a convenience method to instantiate and return an instance of the class using the root element provided. This will be used within `rs-auto-init`, and in the future its presence may be enforced via a custom lint rule.|

#### Foundation Lifecycle handling

`RSComponent` calls its foundation's `init()` function within its _constructor_, and its foundation's `destroy()` function within its own _destroy()_ function. Therefore it's important to remember to _always call super() when overriding destroy()_. Not doing so can lead to leaked resources.
...
```js
class MyComponent extends RSComponent {
